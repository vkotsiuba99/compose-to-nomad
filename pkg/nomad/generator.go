...
import (
	...
	"github.com/vkotsiuba99/compose-to-nomad/pkg/nomadtemplate"
...
func ParseTemplate() *template.Template {
	...
	tmpl, err := template.New("nomad").Funcs(funcMap).Parse(nomadtemplate.JobTemplate)
	if err != nil {
...
func prepareJobData(name string, service types.ServiceConfig) map[string]interface{} {
	...
	for _, portMapping := range service.Ports {
		...
		if len(parts) == 2 {
			...
			if err != nil {
				log.Fatalf("Invalid host port '%s' for service '%s': %v",
					parts[0], name, err)
			}
...
func writeNomadJobFile(filePath string, jobData map[string]interface{}, tmpl *template.Template) {
	...
	if err != nil {
		log.Fatalf("Failed to create output file for service '%s': %v",
			jobData["Name"], err)
	}
	...
	if err != nil {
		log.Fatalf("Failed to execute template for service '%s': %v",
			jobData["Name"], err)
	}
}
